...
// System.out.println("non-leaf node, insert key: " + node1.keys[node1.number - 1] + " " + node2.keys[node2.number - 1] + "direct insert");
			...
			// The maximum value of the left node can be inserted directly, therefore the right
...
// System.out.println("non-leaf node, insert key: " + node1.keys[node1.number - 1] + " " + node2.keys[node2.number - 1] + ", need to split") ;
			...
			// After the non-leaf node split, the parent node pointer of its child node should be updated correctly.
			tempNode.number = this.number - middle;
			tempNode.parent = this.parent;
			// If the parent node is empty, create a new non-leaf node as the parent node, and update the pointers to the new parent node correctly.
			if (this.parent == null) {
...
// System.out.println("non-leaf node, insert key: " + node1.keys[node1.number - 1] + " " + node2.keys[node2.number - 1] + ", create new parent node") ;
			...
			// After the leaf node is successfully split, the newly generated node should be inserted into the parent node.
...
//            System.out.println();
			...
			if (this.number <= bTreeOrder) {
				...
				// It is showing a potential case that although there is no node split, the value actually
				// inserted is greater than the original maximum value, so the boundary values ​​of
